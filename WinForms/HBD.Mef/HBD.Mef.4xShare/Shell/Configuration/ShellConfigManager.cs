...
{
    public abstract class ShellConfigManager<TShellConfig, TModuleConfig> :
        IShellConfigManager<TShellConfig, TModuleConfig>
        where TShellConfig : ShellConfig, new() where TModuleConfig : ModuleConfig, new()
    {
        ...
        public virtual ModuleConfigCollection<TModuleConfig> Modules
            => SingletonManager.GetOrLoad(ref _modules, () => LoadModuleFrom(ShellConfig.ModulePath));
        ...
        public virtual void ImportShellBinaries(AggregateCatalog catalog, ReflectionContext reflectionContext = null)
        {
            ...
            foreach (var s in ShellConfig.ImportedBinaries)
            {
                ...
                var ac = Assembly.LoadFrom(file);
                catalog.Catalogs.Add(CreateAssemblyCatalog(ac,reflectionContext));
            }
        }
        ...
        /// </summary>
        /// <param name="catalog"></param>
        public virtual void ImportModuleBinaries(AggregateCatalog catalog, ReflectionContext reflectionContext = null)
        {
            Guard.ArgumentIsNotNull(catalog, nameof(catalog));
            ...
            foreach (var item in Modules.Where(m => m.IsEnabled && m.IsValid && m.AssemplyFiles.Count <= 0))
                catalog.Catalogs.Add(CreateMultiDirectoriesCatalog(item.Directory, reflectionContext));
            ...
            foreach (var item in Modules.Where(m => m.IsEnabled && m.IsValid && m.AssemplyFiles.Count > 0))
            {
                ...
                var assemblies = from f in item.AssemplyFiles
                                 ...
                                 select CreateAssemblyCatalog(a,reflectionContext);
    ...
    public class ShellConfigManager : ShellConfigManager<ShellConfig, ModuleConfig>, IShellConfigManager
    {
    }
}